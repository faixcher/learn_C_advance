...
struct user{
	...
	int highscore;
	char name[100];
	int (*current_game) (); //function's pointer 
	 
};

// Function prototypes
int get_player_data();
void register_new_play();
...
int dealer_no_match();
int find_the_ace();
void fatal(char *); //Glabal variables struct user player; // Player struct 
struct user player;
int main(){ 
	...
	while(choice != 7){
		...
		printf("2 - Play the No Mathc Dealer game\n");
		...
		if((choice < 1) || (choice > 7))
			...
			if(choice != last_game){
				if(choice == 1)
					player.current_game = pick_a_number; // no() is address of function 
				else if(choice == 2) 
...
int get_player_data(){
	...
	read_bytes = read(fd,&entry,sizeof(struct user)); // Read the frist chuck. 
	
	while(entry.uid != uid && read_bytes > 0){ // loop until proper uid is found 
		read_bytes = read(fd,&entry,sizeof(struct user));//  Keep reading a chunk 
	}
	close(fd);
	if(read_bytes < sizeof(struct user)) // This means that the end of file was reached. 
		return -1;	
	else 
		player = entry; // Copy the read entry into the plaer struct.
	return 1 ;			// Return a success.
}

// This is the new user registration function. 
// It will create a new player account and append it ot the file.
...
void update_player_data(){
	...
	read(fd,&read_uid,4);	//Read the uid from rist struct.
	while(read_uid != player.uid){
		for(i=0;i<sizeof(struct user) -4;i++)	//Read the uid from byte 4+ 
			read(fd,&burned_byte,1); // points to save value without uid 
		read(fd,&read_uid,4);// Read from the first Struct
	}
...
//This function will display the current high socre and
...
void show_highscore(){
	...
	fd = open(DATAFILE,O_RDONLY);
	if(fd == -1)
		fatal("in show highscore() while opening file");
	while(read(fd,&entry,sizeof(struct user)) > 0){	//loop until end of file. if read = 0 
		if(entry.highscore > top_score){     // IF there is a higher score,
			top_score = entry.highscore; // set top_score to that score
			strcpy(top_name,entry.name); // add top_name to that username
		}
	}
	close(fd);
...
//scaf("%s",&whatever) will stop input at the first space.
void input_name() {
	char *name_ptr, input_char='\n';
	while(input_char == '\n')
	       scanf("%c",&input_char);	 

	name_ptr = (char *) &(player.name); // name_ptr points to addree of player.name
	while(input_char != '\n'){
		*name_ptr = input_char;
		scanf("%c",&input_char);
		name_ptr++;
	}
	*name_ptr = 0; // Terninate the stirng with null byte;
}

//This function prints the 3 cards for the Find the Ace game 
//It expects a message to display, a pointer to the cardsarray,
...
// for the second wager in the Find the Ace game. hte function 
// return -1 if the wager is too big or too little, and it returns 
// the wager amount otherwise.
int take_wager(int available_credits, int previous_wager){
	int wager, total_wager;

	printf("How many of your %d credits would you likek to wager? ",available_credits);
	scanf("%d",&wager);
	...
	if(total_wager > available_credits){
		printf("Your total wager of %d is more taht you have!\n",total_wager);
...
void play_the_game(){
	int play_again = 1;
	int (*game) (); // function pointer
	char selection;

	while(play_again){
		printf("\n[DEBUG] current_game pointer @ 0x%016x\n",player.current_game);
		if(player.current_game() != -1){ // if the game plays wihtout error and 
			if(player.credits > player.highscore) 
				player.highscore = player.credits; // a new high score is set 
			printf("\nYou now have %u credits\n",player.credits); // update the highscore
			update_player_data();
			...
			play_again = 0 ; // this mean if player.current_game == -1 , mean returned an error
	}
}
...
int pick_a_number(){
	...
	printf("\n##### Pick a Number #####\n");
	printf("This game costs 10 credtis to play. Simply pick a number\n");
	printf("between 1 and 20 , and if you pick the winning number, you \n");
	printf("will win the jackpot of 100 credits!\n\n");
	winning_number = (rand() %20) + 1; // this all ways use +1 for (0 - 20)
...
//It return -1 if the player has 0 credits.
int dealer_no_match(){
	int i,j,numbers[16],wager = -1,match = -1;

	printf("\n:::::: No Mathc Dealer ::::::\n");
	printf("In this game, you can wager up to all of your credits.\n");
	printf("The dealer will deal out 16 random numbet between 0 and 99.\n");
	printf("if there no matches among them,you double your money!\n\n");

	if(player.credits == 0){
		printf("You din't have any credits to wager!\n\n");
		return -1;
	}
	while (wager == -1)
	...
	for(i=0; i<16;i++){
		...
		printf("%2d\t",numbers[i]);
		if(i%8 == 7)
			printf("\n"); // printf a line break every 8 numbers
	}
	...
	if(match != -1){
		...
		printf("There were no matches! You win %d credtis!\n",wager);
		player.credits += wager;
	}
	return 0;
}
...
//It return -1 if the plaer has 0 credits.
int find_the_ace(){
	...
	printf("******* Find The Ace ********\n");
	printf("In this game, you ca wager up to all of your credtis.\n");
	...
	if(player.credits == 0) {
		printf("You don't have any credtis to wager!\n\n");
		return -1;	
	}
	...
	invalid_choice =1 ;
	while(invalid_choice = 1){
		printf("Would you like to :\n[c]hange you pick\tor\t[i]ncrease your wager?\n");
	...
	if(pick == ace){
		...
		if(wager_two != -1){
			printf("and an additional %d credits from yout second wager!\n",wager_two);
		...
		player.credits -= wager_one;
		if(wager_two != -1){
			printf("and ad additional %d credits from your second wager!\n",wager_two);
			player.credits -= wager_two;
		}
	}
	return 0;
